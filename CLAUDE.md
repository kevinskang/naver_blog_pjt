...
**목표:** Playwright 브라우저 자동화 모듈과 MCP 서버 레이어의 견고한 개발, 품질 테스트 및 린트/타입 유지를 완전 자동화합니다.

**트리거:** 네이버 블로그 기능 추가, 자동화 로직 수정, MCP 도구 연동, 테스트 수행 및 리팩토링 요청 시 `naver-blog-mcp-orchestrator` 스킬을 사용하라. 단순 질문은 직접 응답 가능.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-07-17 | 초기 구성 | 전체 | 하네스 설계안 승인에 따른 신규 구축 |
